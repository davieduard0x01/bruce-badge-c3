...
#include "soc/spi_reg.h"
    ...
    !defined(CONFIG_IDF_TARGET_ESP32) && !defined(CONFIG_IDF_TARGET_ESP32C5) &&                              \
...
#define GPIO_DIR_MASK                                                                                        \
    ((1 << TFT_D0) | (1 << TFT_D1) | (1 << TFT_D2) | (1 << TFT_D3) | (1 << TFT_D4) | (1 << TFT_D5) |         \
     (1 << TFT_D6) | (1 << TFT_D7))
...
#define set_mask(C)                                                                                          \
    xset_mask[C] // 63fps Sprite rendering test 33% faster, graphicstest only 1.8% faster than shifting in
                 // real time

// Real-time shifting alternative to above to save 1KByte RAM, 47 fps Sprite rendering test
/*#define set_mask(C) (((C)&0x80)>>7)<<TFT_D7 | (((C)&0x40)>>6)<<TFT_D6 | (((C)&0x20)>>5)<<TFT_D5 |
(((C)&0x10)>>4)<<TFT_D4 | \
                      (((C)&0x08)>>3)<<TFT_D3 | (((C)&0x04)>>2)<<TFT_D2 | (((C)&0x02)>>1)<<TFT_D1 |
(((C)&0x01)>>0)<<TFT_D0
//*/

// Write 8 bits to TFT
#define tft_Write_8(C)                                                                                       \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)(C));                                                              \
    WR_H
...
#define tft_Write_16(C)                                                                                      \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)(((C) & 0xF800) >> 8));                                            \
    WR_H;                                                                                                    \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)(((C) & 0x07E0) >> 3));                                            \
    WR_H;                                                                                                    \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)(((C) & 0x001F) << 3));                                            \
    WR_H
...
#define tft_Write_16(C)                                                                                      \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)((C) >> 0));                                                       \
    WR_H
#define tft_Write_16S(C)                                                                                     \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)((C) >> 8));                                                       \
    WR_H
#else
...
#define tft_Write_32(C)                                                                                      \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)((C) >> 24));                                                      \
    WR_H;                                                                                                    \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)((C) >> 16));                                                      \
    WR_H;                                                                                                    \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)((C) >> 8));                                                       \
    WR_H;                                                                                                    \
...
#define tft_Write_32C(C, D)                                                                                  \
    ...
    WR_H;                                                                                                    \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)((D) >> 8));                                                       \
    WR_H;                                                                                                    \
    GPIO.out_w1tc.val = GPIO_OUT_CLR_MASK;                                                                   \
    GPIO.out_w1ts.val = set_mask((uint8_t)((D) >> 0));                                                       \
    WR_H
